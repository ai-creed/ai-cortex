...
4. **On harness timeout / kill — fail-open?**
   ...
   - **Claude: undocumented.** Whether a timed-out PreToolUse hook fails open or closed is not in the public docs. This is an **open risk** (§13), not an assumption. Mitigation until verified: layers 1–2 keep runtime far below the timeout in practice; the hook always `exit 0` and never emits a `deny`; the explicit 5 s install timeout bounds worst-case user wait. If Claude is found to fail *closed* on timeout, that is a release blocker for the Claude path and must be handled before enabling it by default.
...
The explicit 5 s timeout bounds worst-case user wait on a hung hook. Codex then fails open (proceeds) by confirmed behavior; Claude's on-timeout behavior is an open risk (§8.1, §13).
...
- **BLOCKER (Codex path): `apply_patch` `tool_input` shape is unverified.** The "patch body is exposed as `tool_input.command`" claim derives from Codex PR #18391's description, **not** the documented payload schema. If the real PreToolUse payload exposes the patch differently, Codex edit-time surfacing silently never works. The Codex path MUST NOT be enabled until a fixture captured from a **real Codex `apply_patch` PreToolUse invocation** is committed and the parser is tested against it (phase-gate in §11). Until then the Codex install entry is gated off.
- **OPEN RISK (Claude path): PreToolUse timeout fail-open vs fail-closed is undocumented.** If Claude fails *closed* on hook timeout, a hung hook would block edits — a release blocker for the Claude path. Must be verified empirically before the Claude path is enabled by default. Mitigations in place: §8.1 layers 1–3, always `exit 0`, never `deny`, explicit 5 s install timeout.
