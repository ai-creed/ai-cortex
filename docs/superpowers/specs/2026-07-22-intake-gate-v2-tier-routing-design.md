...
2. **A new harvest, taken before the Phase 2 backlog drain:** the current 600+ untriaged candidates still hold their full raw bodies. The curation pass labels a **tier-blind sample drawn from both tiers** — every high-tier survivor PLUS all zero-signal candidates up to a minimum of 100 (all of them if fewer exist) — judging gem/noise from the body alone, without seeing the tier score. This deliberately hunts for gems in the only population the new routing can destroy. Every labeled gem, whatever its current tier, becomes a keeper case with its full raw body; the labeling outcome for the zero-signal sample (count inspected, gems found) is recorded in the fixture file. Finding zero gems in the zero-signal sample is a legitimate outcome, but it must be the recorded result of the blind pass, never an unexamined assumption. This is the only window where full-fidelity bodies exist — after the drain they are gone.
3. Synthetic cases for each gate rule and signal marker (already the fixture pattern).

**The release gate consumes the snapshot corpus, deterministically, in CI:** junk suppression ≥ 80% across labeled noise; **gem loss = 0 across full-fidelity labeled keepers, hard, with no waiver path.** A keeper that scores low-tier fails CI; the remedy is always tuning `signalScore` markers, never weakening the gate or relabeling the case. There is no runtime or fixture-annotation waiver mechanism — an autonomous implementation must not be able to self-authorize a loss. The single pre-authorized exception, granted by this spec itself (the externally approved artifact that predates the implementation workflow): the 11 excerpt-fidelity keepers from the 2026-06-08 corpus are excluded from the hard gate population — their text is a truncated provenance excerpt, not the intake body — and are reported advisory-only, listed in the fixture with the `excerpt-fidelity` reason. No other exclusions may be added during implementation.
...
8. Replay: snapshot corpus assertions deterministic in CI (these are the release gates); every full-fidelity keeper asserts high-tier scoring individually, so a single lost gem names its case in the failure output; the fixture's recorded zero-signal blind-pass result (count inspected, gems found) is asserted present and non-empty as a structural precondition of the gate; live-corpus advisory mode exercised locally before release.

## 10. Acceptance criteria

1. The committed snapshot corpus meets both gates in CI: ≥ 80% suppression on labeled noise, and **zero** gem loss on full-fidelity labeled keepers with no waiver mechanism (the sole exclusion is the spec-pre-authorized excerpt-fidelity class, reported advisory-only).
2. The pre-drain harvest is committed: a tier-blind labeling pass covering every high-tier survivor plus at least 100 zero-signal candidates (or all, if fewer), its raw-body gem cases and its recorded zero-signal outcome frozen into the fixture corpus **before** Phase 2 deletes their only full-fidelity source.
