...
- Survivors stay `candidate` **and are retyped to `type:"capture"`** via a **new dedicated lifecycle method** `retypeCandidate(lc, id, newType)` (no existing API does this — `updateMemory` cannot change `type`; `rewriteMemory` changes `type` but force-promotes `candidate→active`). `retypeCandidate`: throws unless `status==='candidate'` (candidate-only by design — it cannot be used to bypass the rewrite/confirm promotion rules); **clears `typeFields` at the type boundary** — `typeFields` is per-type metadata (e.g. `gotcha.severity`); carrying the old type's fields across a type change is always wrong, and `validateRegistration` only checks *declared* extras so it would not strip a stale `severity` (it would silently survive, then a later `rewrite_memory` to `decision` with `typeFields` omitted preserves it via `nextTypeFields = fields.typeFields ?? current.typeFields`, leaking irrelevant legacy metadata). So `retypeCandidate` validates `(newType, {})` and commits with `typeFields` reset to `{}` (or omitted; `capture` declares no `extraFrontmatter`). Net for legacy: any inherited `gotcha.severity`/etc. is dropped at retype-to-`capture`; the agent's later `rewrite_memory` to the real type supplies that type's correct `typeFields` from a clean slate. Commits a new version with `type=newType`, `typeFields={}`, `status` **unchanged** (`candidate`), body unchanged, audit `changeType:"retype"`. **`"retype"` is a new arm on the closed `AuditChangeType` union (`types.ts:78`)** — adding it is part of this work; any exhaustive `switch`/consumer of `AuditChangeType` must gain the arm (no `default`-swallow). Triage calls it with `newType:"capture"`; it is a general candidate-retype primitive but in this feature only triage uses it. Their old inferred type was an unreliable regex guess we explicitly distrust, and retyping makes the unreviewed set uniform: fresh and legacy survivors are indistinguishable to the queue, briefing, and browser — all `source='extracted' AND status='candidate' AND type='capture'`. The `review_pending_captures` predicate's source of truth remains `source='extracted' AND status='candidate'`; `type='capture'` is now the consistent label across that whole set, not just fresh rows. Legacy survivors usually resolve to `context.kind: "body-only"` (sessions pruned); the agent judges from the body and assigns the real type via `rewrite_memory`.
...
| Registry seed migration | persisted `types.json` at old version lacking `capture` → after load/ensure, `capture` present + `version==REGISTRY_VERSION`; **non-`capture` user-defined types untouched; a same-name user `capture` is overwritten with the built-in spec + one-time diagnostic emitted**; idempotent on re-run; brand-new repo seeded directly | unit |
| `retypeCandidate` | candidate-only (throws on non-candidate); validates `(newType,{})` vs registry; commits new version `type=newType`, **`typeFields` reset to `{}`** (seed a `gotcha` w/ `severity`, retype→`capture`, assert `severity` gone; then `rewrite_memory`→`decision` omitting typeFields → still clean), `status` stays `candidate`, body unchanged, audit `changeType:"retype"` | unit (reuse lifecycle fixtures) |
...
- **Pre-existing sharp edge (out of scope, severed here): `rewriteMemory` with a changed `type` but omitted `typeFields` preserves the old type's fields** (`nextTypeFields = fields.typeFields ?? current.typeFields`). This feature does not fix that general behavior, but `retypeCandidate` resetting `typeFields` to `{}` at the retype-to-`capture` boundary means every capture entering the agent's `rewrite_memory` step starts from a clean slate, so the leak cannot manifest in this flow. Flagged so the plan does not "fix" `rewriteMemory` (scope creep) yet knows the reset is load-bearing.
...
src/
  lib/memory/
    ...
    registry.ts             (+ "capture" reserved built-in; REGISTRY_VERSION→2; idempotent version<N seed-merge — union; force-overrides same-name user "capture" w/ diagnostic)
    types.ts                (+ "retype" arm on AuditChangeType union; update exhaustive consumers)
    lifecycle.ts            (+ retypeCandidate(lc,id,newType): candidate-only, registry-validated, status-preserving, typeFields reset to {}; audit changeType "retype")
    capture-triage.ts       (new: one-shot legacy triage migration + sentinel; uses retypeCandidate to stamp survivors type:"capture")
