...
| `retypeCandidate` | candidate-only (throws on non-candidate); validates `(newType,typeFields)` vs registry; commits new version with `type=newType`, `status` stays `candidate`, body unchanged, audit `changeType:"retype"` | unit (reuse lifecycle fixtures) |
...
src/
  lib/memory/
    ...
    lifecycle.ts            (+ retypeCandidate(lc,id,newType): candidate-only, registry-validated, status-preserving; audit changeType "retype")
