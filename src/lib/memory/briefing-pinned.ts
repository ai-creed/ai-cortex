...
export async function renderPinnedSection(repoKey: string): Promise<string | null> {
    const cfg = await loadMemoryConfig(repoKey);
    const rh = openRetrieve(repoKey);
    try {
        const explicit = rh.index.rawDb().prepare(`
        ...
        `).all() as Array<{ id: string; type: string; status: string; title: string; bodyExcerpt: string; updatedAt: string; confidence: number }>;

        const auto = rh.index.rawDb().prepare(`
            SELECT id, type, status, title, body_excerpt AS bodyExcerpt, updated_at AS updatedAt, confidence
            FROM memories
