...
export async function writeMemoryFile(repoKey: string, record: MemoryRecord): Promise<void> {
...
export async function readMemoryFile(repoKey: string, id: string, location: "memories" | "trash"): Promise<MemoryRecord> {
	const text = await fs.readFile(memoryFilePath(repoKey, id, location), "utf8");
	return parseMemoryMarkdown(text);
}

export async function listMemoryFiles(repoKey: string): Promise<MemoryFileEntry[]> {
	await ensureDirs(repoKey);
...
export async function restoreFromTrash(repoKey: string, id: string): Promise<void> {
...
export async function purgeMemoryFile(repoKey: string, id: string, location: "memories" | "trash"): Promise<void> {
	try {
