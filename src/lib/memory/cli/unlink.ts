...
const VALID_REL_TYPES: MemoryEdgeType[] = ["supports", "contradicts", "refines", "depends_on"];
...
function parseUnlinkArgs(args: string[]): UnlinkArgs {
	...
	for (let i = 0; i < args.length; i++) {
		const a = args[i];
		if (a === "--type" && args[i + 1]) { relType = args[++i]; continue; }
	...
	return { srcId: positional[0], dstId: positional[1], relType: relType as MemoryEdgeType };
}

export async function runMemoryUnlink(args: string[], opts: {
	repoKey: string;
