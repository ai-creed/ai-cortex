...
export class MemoryIndex {
	...
	rawAllTables(): string[] {
		return (this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' OR type='virtual'").all() as { name: string }[]).map(r => r.name);
	}
	...
	upsertMemory(fm: MemoryFrontmatter, opts: { bodyHash: string; bodyExcerpt: string; body: string }): void {
		const tx = this.db.transaction(() => {
			...
			`).run({
				id: fm.id, type: fm.type, status: fm.status, title: fm.title, version: fm.version,
				createdAt: fm.createdAt, updatedAt: fm.updatedAt, source: fm.source, confidence: fm.confidence,
				pinned: fm.pinned ? 1 : 0, bodyHash: opts.bodyHash, bodyExcerpt: opts.bodyExcerpt,
			});

			this.db.prepare("DELETE FROM memory_scope WHERE memory_id = ?").run(fm.id);
			const insScope = this.db.prepare("INSERT INTO memory_scope (memory_id, kind, value) VALUES (?, ?, ?)");
			...
			this.db.prepare("INSERT INTO memory_fts (memory_id, title, body) VALUES (?, ?, ?)").run(fm.id, fm.title, opts.body);
		});
		tx();
	}
	...
	scopeRows(id: string): ScopeRow[] {
		return this.db.prepare("SELECT kind, value FROM memory_scope WHERE memory_id = ? ORDER BY kind, value").all(id) as ScopeRow[];
	}
	...
	removeLink(srcId: string, dstId: string, relType: string): void {
		this.db.prepare("DELETE FROM memory_links WHERE src_id = ? AND dst_id = ? AND rel_type = ?").run(srcId, dstId, relType);
	}
