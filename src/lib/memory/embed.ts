...
export async function upsertMemoryVector(
	...
	const entries = existing?.meta.entries.filter(e => e.path !== entryPathFor(memoryId)) ?? [];
	const matrixSize = (entries.length + 1) * dim;
	const matrix = new Float32Array(matrixSize);

	if (existing) {
		for (let i = 0; i < entries.length; i++) {
			const oldIdx = existing.meta.entries.findIndex(e => e.path === entries[i]!.path);
			matrix.set(existing.matrix.slice(oldIdx * dim, (oldIdx + 1) * dim), i * dim);
		}
	}
...
export async function readMemoryVector(repoKey: string, memoryId: string): Promise<MemoryVector | null> {
	const dir = memoryRootDir(repoKey);
	...
	return { memoryId, vector: vec, dim: idx.meta.dim, bodyHash: idx.meta.entries[i]!.hash };
}

export async function deleteMemoryVector(repoKey: string, memoryId: string): Promise<void> {
	const dir = memoryRootDir(repoKey);
	const idx = await readVectorIndex(dir, MODEL_NAME);
	if (!idx) return;
	const target = entryPathFor(memoryId);
	...
	await writeVectorIndex(dir, { matrix, meta: { modelName: MODEL_NAME, dim, count: keep.length, entries: keep } });
}
