...
export async function installHooks(opts: InstallOpts): Promise<"installed" | "no-op" | "aborted"> {
	...
	if (afterText.trim() === before.trim() && codexAfterText.trim() === codexBefore.trim()) {
	...
	if (!opts.yes) {
		...
		printDiff("~/.codex/config.toml", codexBefore.length > 0 ? codexBefore : "(no existing file)\n", codexAfterText);
		const proceed = await confirm("Apply these changes to Claude and Codex hook config? [y/N] ", opts.answerForTest);
		if (!proceed) return "aborted";
	}

	if (before.length > 0) {
		const ts = new Date().toISOString().replace(/[:.]/g, "-");
		fs.copyFileSync(settingsPath, path.join(path.dirname(settingsPath), `settings.json.bak.${ts}`));
	}
	if (codexBefore.length > 0 && codexAfterText.trim() !== codexBefore.trim()) {
		const ts = new Date().toISOString().replace(/[:.]/g, "-");
		fs.copyFileSync(codexPath, path.join(path.dirname(codexPath), `config.toml.bak.${ts}`));
	}
...
export async function uninstallHooks(opts: InstallOpts): Promise<"uninstalled" | "no-op" | "aborted"> {
	...
	if (afterText.trim() === before.trim() && codexAfterText.trim() === codexBefore.trim()) return "no-op";
...
function codexEventHasMarker(text: string, evt: string): boolean {
	...
	return groups.some((group) => group.event === evt && group.lines.join("\n").includes(HOOK_COMMAND_MARKER));
}
...
function codexHookGroups(text: string): Array<{ event: string; start: number; end: number; lines: string[] }> {
	const lines = text.split("\n");
	const groups: Array<{ event: string; start: number; end: number; lines: string[] }> = [];
...
function removeCodexMarkerHooks(lines: string[]): string[] {
	...
	if (hookStarts.length === 0) return lines.join("\n").includes(HOOK_COMMAND_MARKER) ? [] : lines;
