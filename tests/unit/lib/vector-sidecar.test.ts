...
describe("writeVectorIndex / readVectorIndex", () => {
	it("round-trips a small VectorIndex", async () => {
		...
		if (!result)
			throw new Error("readVectorIndex returned null for existing sidecar");
	...
	it("throws VectorIndexCorruptError when .bin is truncated", async () => {
		...
		await expect(
			readVectorIndex(tmpDir, "Xenova/all-MiniLM-L6-v2"),
		).rejects.toThrow(VectorIndexCorruptError);
	});
