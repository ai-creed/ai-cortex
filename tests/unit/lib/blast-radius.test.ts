...
const calls: CallEdge[] = [
	...
	{
		from: "src/suggest.ts::suggestRepo",
		to: "src/ranker.ts::rankFiles",
		kind: "call",
	},
	{
		from: "src/ranker.ts::rankFiles",
		to: "src/ranker.ts::scoreItem",
		kind: "call",
	},
];
...
describe("queryBlastRadius", () => {
	...
	it("returns transitive callers at hop 2+", () => {
		...
		expect(result.tiers[1]?.hits).toContainEqual(
			expect.objectContaining({ qualifiedName: "main", hop: 2 }),
		);
	});
...
describe("queryBlastRadius — overload aggregation", () => {
	...
	it("omits overloadCount when target identity is unambiguous", () => {
		...
		expect(
			result.overloadCount === undefined || result.overloadCount === 1,
		).toBe(true);
	});
});
