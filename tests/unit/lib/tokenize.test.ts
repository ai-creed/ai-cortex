...
describe("tokenize (path/identifier mode — no stopword filter)", () => {
	it("splits camelCase and keeps joined form", () => {
		expect(tokenize("CardView")).toEqual(
			expect.arrayContaining(["card", "view", "cardview"]),
		);
	});
	...
	it("splits PascalCase with multiple segments", () => {
		...
		expect(toks).toEqual(
			expect.arrayContaining(["my", "work", "panel", "myworkpanel"]),
		);
	});
	...
	it("handles ALLCAPS boundary without splitting chars", () => {
		...
		expect(toks).toEqual(
			expect.arrayContaining(["xml", "parser", "xmlparser"]),
		);
	...
	it("tokenizes a full path with separators and extension", () => {
		...
		expect(toks).toEqual(
			expect.arrayContaining(["src", "features", "my", "work", "card", "tsx"]),
		);
	});
...
describe("tokenizeTask (task mode — stopword filter applied)", () => {
	...
	it("stems '-ies' to '-y' (flies → fly)", () => {
		expect(tokenizeTask("flies")).toEqual(
			expect.arrayContaining(["flies", "fly"]),
		);
	});

	it("stems '-xes/-ches/-shes/-sses/-zes' by trimming 'es'", () => {
		expect(tokenizeTask("boxes")).toEqual(
			expect.arrayContaining(["boxes", "box"]),
		);
		expect(tokenizeTask("matches")).toEqual(
			expect.arrayContaining(["matches", "match"]),
		);
		expect(tokenizeTask("hashes")).toEqual(
			expect.arrayContaining(["hashes", "hash"]),
		);
	});
	...
	it("still splits camelCase in task strings", () => {
		...
		expect(toks).toEqual(
			expect.arrayContaining(["create", "card", "createcard"]),
		);
	});
