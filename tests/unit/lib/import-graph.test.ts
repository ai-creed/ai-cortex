...
describe("import-graph — C/C++ basename fallback", () => {
	...
	beforeAll(() => {
		...
		fs.writeFileSync(path.join(dir, "lib", "utils.h"), "int helper();\n");
	});
	...
	it("drops include when no file with matching basename exists in allFilePaths", async () => {
		...
		const edges = await extractImports(dir, ["src/main.cpp"], ["src/main.cpp"]);
		expect(edges).toEqual([]);
	});
...
describe("import-graph — Python resolveSite", () => {
	...
	it("resolves package directory via __init__.py", async () => {
		...
		fs.writeFileSync(path.join(dir, "mypackage", "sub", "__init__.py"), "");
		const edges = await extractImports(
			dir,
			["mypackage/models.py"],
...
describe("extractImports — contentMap skips file reads", () => {
	it("performs zero fs.readFileSync calls when contentMap is provided", async () => {
		...
		const contentMap = new Map([["src/a.ts", 'import { b } from "./b";']]);
		const edges = await extractImports(
			"/fake",
			["src/a.ts"],
			["src/a.ts", "src/b.ts"],
