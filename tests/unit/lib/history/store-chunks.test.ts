...
describe("writeAllChunks + getChunkText", () => {
	it("round-trips a single chunk", async () => {
		await writeAllChunks("REPO", "abc", [{ id: 0, text: "hello world" }]);
		expect(await getChunkText("REPO", "abc", 0)).toBe("hello world");
	});
	...
	it("writes multiple chunks readable by id", async () => {
		...
		expect(await getChunkText("REPO", "abc", 0)).toBe("first");
		expect(await getChunkText("REPO", "abc", 1)).toBe("second");
	});
	...
	it("returns null when chunks file absent", async () => {
		expect(await getChunkText("REPO", "abc", 0)).toBeNull();
	});

	it("returns null when id not present", async () => {
		await writeAllChunks("REPO", "abc", [{ id: 0, text: "x" }]);
		expect(await getChunkText("REPO", "abc", 99)).toBeNull();
	});
