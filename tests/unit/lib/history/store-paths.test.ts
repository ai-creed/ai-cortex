...
import { historyDir, sessionDir, sessionJsonPath, chunksJsonlPath, lockPath } from "../../../../src/lib/history/store.js";

describe("store paths", () => {
	it("historyDir uses getCacheDir + 'history'", () => {
		vi.spyOn(os, "homedir").mockReturnValue("/home/u");
		expect(historyDir("REPO")).toBe(path.join("/home/u", ".cache", "ai-cortex", "v1", "REPO", "history"));
	});

	it("sessionDir is sessions/<id> under historyDir", () => {
		vi.spyOn(os, "homedir").mockReturnValue("/home/u");
		expect(sessionDir("REPO", "abc")).toBe(
			path.join("/home/u", ".cache", "ai-cortex", "v1", "REPO", "history", "sessions", "abc"),
		);
	});
