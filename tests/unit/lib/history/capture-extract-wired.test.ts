...
const TRANSCRIPT_LINES = [
	{
		type: "user",
		turn: 1,
		message: { content: "always use POST for create endpoints" },
	},
...
describe("captureSession — extractor wiring", () => {
	...
	it("captureSession invokes extractor after compaction", async () => {
		const tp = path.join(
			"/tmp",
			`aicortex-test-transcript-${Date.now()}.jsonl`,
		);
		...
		expect(vi.mocked(extractFromSession)).toHaveBeenCalledWith(
			repoKey,
			"s-cap-1",
		);
	});
	...
	it("does not fail capture if the extractor throws", async () => {
		const tp = path.join(
			"/tmp",
			`aicortex-test-transcript-${Date.now()}-2.jsonl`,
		);
		await writeFakeTranscript(tp);
