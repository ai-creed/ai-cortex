...
describe("deriveSlug", () => {
	it("kebab-cases simple titles", () => {
		expect(deriveSlug("Cache writes use atomic temp-file rename"))
			.toBe("cache-writes-use-atomic-temp-file-rename");
	});

	it("strips punctuation and collapses whitespace", () => {
		expect(deriveSlug("Hello, World! — what a `title`"))
			.toBe("hello-world-what-a-title");
	});

	it("caps slug at 40 chars on word boundary when possible", () => {
		const long = "this title is intentionally written to be way longer than the cap allows";
...
describe("generateMemoryId", () => {
	it("matches mem-YYYY-MM-DD-<slug>-<6hex>", () => {
		const id = generateMemoryId("Cache writes", new Date("2026-04-30T09:18:44Z"));
	...
	it("produces 6-hex suffixes that are well-distributed across small batches", () => {
		...
		for (let i = 0; i < 200; i++) {
			seen.add(generateMemoryId("same title", new Date("2026-04-30T00:00:00Z")));
		}
