...
import {
	createMemory, openLifecycle, updateMemory, updateScope,
	deprecateMemory, restoreMemory, mergeMemories,
	trashMemory, untrashMemory,
	purgeMemory,
	linkMemories, unlinkMemories,
	pinMemory, unpinMemory, confirmMemory,
	addEvidence,
...
describe("trashMemory / untrashMemory", () => {
	it("moves file to trash/, flips status to trashed, records audit", async () => {
		...
		try {
			...
			const trashed = await fs.access(memoryFilePath(repoKey, id, "trash")).then(() => true).catch(() => false);
			const stillInMemories = await fs.access(memoryFilePath(repoKey, id, "memories")).then(() => true).catch(() => false);
	...
	it("untrashMemory restores file to memories/ and flips status back to active", async () => {
		...
		try {
			...
			expect(lc.index.getMemory(id)!.status).toBe("active");
			const restored = await fs.access(memoryFilePath(repoKey, id, "memories")).then(() => true).catch(() => false);
...
describe("purgeMemory (default)", () => {
	it("hard-deletes the .md, removes FTS, leaves audit + memories row", async () => {
		const lc = await openLifecycle(repoKey, { agentId: "test" });
		try {
			const id = await createMemory(lc, { type: "decision", title: "T", body: "## Rule\nx", scope: { files: [], tags: [] }, source: "explicit" });
...
describe("purgeMemory (redact)", () => {
	it("scrubs title + body_excerpt, nulls audit prev_body, removes FTS + vector", async () => {
		const lc = await openLifecycle(repoKey, { agentId: "test" });
		try {
			const id = await createMemory(lc, { type: "decision", title: "Sensitive decision", body: "## Rule\nsecret", scope: { files: [], tags: [] }, source: "explicit" });
	...
	it("merge case: redact src only; dst body unchanged", async () => {
		const lc = await openLifecycle(repoKey, { agentId: "test" });
		try {
			const a = await createMemory(lc, { type: "decision", title: "A", body: "## Rule\na-secret", scope: { files: [], tags: [] }, source: "explicit" });
			const b = await createMemory(lc, { type: "decision", title: "B", body: "## Rule\nb-public", scope: { files: [], tags: [] }, source: "explicit" });
			await mergeMemories(lc, a, b, "## Rule\nmerged");
...
describe("linkMemories / unlinkMemories", () => {
	...
	it("rejects unknown rel types", async () => {
		...
		try {
			const a = await createMemory(lc, { type: "decision", title: "A", body: "## Rule\na", scope: { files: [], tags: [] }, source: "explicit" });
			const b = await createMemory(lc, { type: "decision", title: "B", body: "## Rule\nb", scope: { files: [], tags: [] }, source: "explicit" });
			await expect(linkMemories(lc, a, b, "ohno" as any)).rejects.toThrow(/rel.*type/i);
		} finally { lc.close(); }
	});
	...
	it("rejects edges referencing missing memories", async () => {
		...
		try {
			const a = await createMemory(lc, { type: "decision", title: "A", body: "## Rule\na", scope: { files: [], tags: [] }, source: "explicit" });
			await expect(linkMemories(lc, a, "mem-fake-x", "supports")).rejects.toThrow(/not found/);
		} finally { lc.close(); }
	});
});
...
describe("pinMemory / unpinMemory", () => {
	...
	it("respects pinnedHardCap; force flag bypasses", async () => {
		...
		try {
			const ids: string[] = [];
			for (let i = 0; i < 21; i++) {
				ids.push(await createMemory(lc, {
					type: "decision", title: `T${i}`, body: "## Rule\nx",
					scope: { files: [], tags: [] }, source: "explicit",
				}));
			}
...
describe("confirmMemory", () => {
	it("flips candidate → active and confidence → 1.0; audit 'promote'", async () => {
		...
		try {
			const id = await createMemory(lc, {
				type: "gotcha", title: "G", body: "## Symptom\nx",
				scope: { files: [], tags: [] }, source: "extracted", confidence: 0.55,
				typeFields: { severity: "warning" },
			});
	...
	it("rejects from non-candidate states", async () => {
		...
		try {
			...
			await expect(confirmMemory(lc, id)).rejects.toThrow(/only from candidate/);
		} finally { lc.close(); }
	});
});
...
describe("addEvidence", () => {
	it("appends a provenance entry to the memory's frontmatter and bumps version", async () => {
		...
		try {
			...
			expect(file.frontmatter.provenance[0]!.sessionId).toBe("s-2026-04-30-abcd");
