...
import { writeManifest, readManifest } from "../../../../src/lib/memory/extract.js";
import { extractorRunPath } from "../../../../src/lib/memory/paths.js";
import { mkRepoKey, cleanupRepo } from "../../../helpers/memory-fixtures.js";

describe("extract — manifest io", () => {
	let repoKey: string;
	beforeEach(async () => { repoKey = await mkRepoKey("manifest"); });
	...
	it("writes a manifest at the expected path and reads it back unchanged", async () => {
		const m = {
			...
			candidatesCreated: 1,
			evidenceAppended: 2,
			rejectedCandidates: [{ type: "decision" as const, reason: "below floor", previewText: "preview" }],
