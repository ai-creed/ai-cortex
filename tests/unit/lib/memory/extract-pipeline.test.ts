...
describe("extractFromSession — end to end", () => {
	let repoKey: string;
	beforeEach(async () => { repoKey = await mkRepoKey("extract-e2e"); });
	...
	it("creates a candidate for an imperative correction", async () => {
		...
		const rows = idx.rawDb().prepare("SELECT id, type, status, confidence FROM memories").all();
	...
	it("appends evidence and bumps confidence on a near-duplicate in a second session", async () => {
		await writeSession(repoKey, {
			...
			evidence: {
				toolCalls: [], filePaths: [{ turn: 4, path: "src/api/create.ts" }],
				userPrompts: [],
				corrections: [{ turn: 4, text: "always use POST for create endpoints" }],
			}, chunks: [],
		});
		...
		await writeSession(repoKey, {
			version: 2, id: "s-2",
			startedAt: "2026-04-30T02:00:00Z", endedAt: "2026-04-30T03:00:00Z",
			turnCount: 1, lastProcessedTurn: 1, hasSummary: false, hasRaw: true,
			rawDroppedAt: null, transcriptPath: "/tmp/y", summary: "",
			evidence: {
				toolCalls: [], filePaths: [{ turn: 2, path: "src/api/update.ts" }],
				userPrompts: [],
				corrections: [{ turn: 2, text: "always prefer POST for create endpoints" }],
			}, chunks: [],
		});
		...
		const idx = openMemoryIndex(repoKey);
		const rows = idx.rawDb().prepare("SELECT id, confidence FROM memories").all() as any[];
		expect(rows).toHaveLength(1);
	...
	it("re-running on a session that has not grown is a no-op (lastProcessedTurn covers all turns)", async () => {
		await writeSession(repoKey, {
			version: 2, id: "s-1",
			startedAt: "2026-04-30T00:00:00Z", endedAt: null,
			turnCount: 1, lastProcessedTurn: 1, hasSummary: false, hasRaw: true,
			rawDroppedAt: null, transcriptPath: "/tmp/x", summary: "",
			evidence: {
	...
	it("re-running after session grows extracts only the new turns", async () => {
		await writeSession(repoKey, {
			version: 2, id: "s-1",
			startedAt: "2026-04-30T00:00:00Z", endedAt: null,
			turnCount: 2, lastProcessedTurn: 2, hasSummary: false, hasRaw: true,
			rawDroppedAt: null, transcriptPath: "/tmp/x", summary: "",
			evidence: {
				toolCalls: [], filePaths: [], userPrompts: [],
				corrections: [{ turn: 1, text: "always run pnpm typecheck" }],
		...
		await writeSession(repoKey, {
			version: 2, id: "s-1",
			startedAt: "2026-04-30T00:00:00Z", endedAt: null,
			turnCount: 6, lastProcessedTurn: 6, hasSummary: false, hasRaw: true,
			rawDroppedAt: null, transcriptPath: "/tmp/x", summary: "",
			evidence: {
				toolCalls: [], filePaths: [], userPrompts: [],
				corrections: [
	...
	it("allowReExtract: true re-scans from turn 0 and overwrites the manifest", async () => {
		...
		await extractFromSession(repoKey, "s-1");
		const r = await extractFromSession(repoKey, "s-1", { allowReExtract: true });
