...
import { writeMemoryFile, readMemoryFile, listMemoryFiles, moveToTrash, restoreFromTrash, purgeMemoryFile } from "../../../../src/lib/memory/store.js";
...
describe("listMemoryFiles", () => {
	it("returns id, location, mtime for every .md under memories/ and trash/", async () => {
		...
		expect(list.map(x => x.id).sort()).toEqual(["mem-2026-04-30-a-100000", "mem-2026-04-30-b-200000"]);
...
describe("moveToTrash / restoreFromTrash / purgeMemoryFile", () => {
	it("moves the file from memories/ to trash/ and back", async () => {
		...
		expect(await fs.access(path.join(memoriesDir(repoKey), `${r.frontmatter.id}.md`)).then(() => true).catch(() => false)).toBe(false);
		expect(await fs.access(path.join(trashDir(repoKey), `${r.frontmatter.id}.md`)).then(() => true).catch(() => false)).toBe(true);
		await restoreFromTrash(repoKey, r.frontmatter.id);
		expect(await fs.access(path.join(memoriesDir(repoKey), `${r.frontmatter.id}.md`)).then(() => true).catch(() => false)).toBe(true);
	});
