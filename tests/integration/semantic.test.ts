...
describe.skipIf(!RUN_SEMANTIC)(
	"semantic integration (requires AI_CORTEX_SEMANTIC_INTEGRATION=1)",
	() => {
		let result: SemanticSuggestResult;

		beforeAll(async () => {
			// First call builds the sidecar — may take 30–60 s on cold start
			const r = await suggestRepo(
				THIS_REPO,
		...
		it("second call with stale:true is faster (uses cached sidecar)", async () => {
			...
			if (r2.mode !== "semantic")
				throw new Error(`unexpected mode: ${r2.mode}`);
