...
describe("ai-cortex memory CLI", () => {
	...
	it("memory get returns JSON with correct id", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Get test", "--body-file", bodyFile, "--repo-key", "test-get"]);
	...
	it("memory get nonexistent id exits with code 1", () => {
		const out = run(["memory", "get", "nonexistent-id", "--repo-key", "test-get-missing"]);
		expect(out.status).toBe(1);
	});

	// ─── Task 8.4 — memory list ──────────────────────────────────────────────
	it("memory list --json returns empty array for fresh repo", () => {
		const out = run(["memory", "list", "--json", "--repo-key", "test-list-empty"]);
		expect(out.status).toBe(0);
		const parsed = JSON.parse(out.stdout);
		expect(Array.isArray(parsed)).toBe(true);
	...
	it("memory list --json returns item after recording", () => {
		...
		run(["memory", "record", "--type", "decision", "--title", "List item", "--body-file", bodyFile, "--repo-key", "test-list-item"]);

		const out = run(["memory", "list", "--json", "--repo-key", "test-list-item"]);
		expect(out.status).toBe(0);
		const parsed = JSON.parse(out.stdout);
	...
	it("memory update changes title and get reflects new title", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Original title", "--body-file", bodyFile, "--repo-key", "test-update"]);
		const id = rec.stdout.trim();

		const upd = run(["memory", "update", id, "--title", "New title", "--reason", "test", "--repo-key", "test-update"]);
		expect(upd.status).toBe(0);
		expect(upd.stdout.trim()).toBe("ok");

		const got = run(["memory", "get", id, "--json", "--repo-key", "test-update"]);
	...
	it("memory deprecate marks memory as deprecated", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Dep test", "--body-file", bodyFile, "--repo-key", "test-deprecate"]);
		const id = rec.stdout.trim();

		const dep = run(["memory", "deprecate", id, "--reason", "replaced", "--repo-key", "test-deprecate"]);
		expect(dep.status).toBe(0);

		const list = run(["memory", "list", "--status", "deprecated", "--json", "--repo-key", "test-deprecate"]);
	...
	it("memory restore brings deprecated memory back to active", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Restore test", "--body-file", bodyFile, "--repo-key", "test-restore"]);
		const id = rec.stdout.trim();

		run(["memory", "deprecate", id, "--reason", "temp", "--repo-key", "test-restore"]);
		const res = run(["memory", "restore", id, "--repo-key", "test-restore"]);
		expect(res.status).toBe(0);

		const list = run(["memory", "list", "--status", "active", "--json", "--repo-key", "test-restore"]);
		const items = JSON.parse(list.stdout);
		expect(items.some((i: { id: string }) => i.id === id)).toBe(true);
	});
	...
	it("memory merge marks src as merged_into", () => {
		...
		const src = run(["memory", "record", "--type", "decision", "--title", "Merge src", "--body-file", b1, "--repo-key", "test-merge"]).stdout.trim();
		const dst = run(["memory", "record", "--type", "decision", "--title", "Merge dst", "--body-file", b2, "--repo-key", "test-merge"]).stdout.trim();

		const mg = run(["memory", "merge", src, dst, "--body-file", bm, "--repo-key", "test-merge"]);
		expect(mg.status).toBe(0);

		const got = run(["memory", "get", src, "--json", "--repo-key", "test-merge"]);
		const parsed = JSON.parse(got.stdout);
	...
	it("memory trash marks memory as trashed", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Trash test", "--body-file", bodyFile, "--repo-key", "test-trash"]);
		const id = rec.stdout.trim();

		const tr = run(["memory", "trash", id, "--reason", "no longer needed", "--repo-key", "test-trash"]);
		expect(tr.status).toBe(0);

		const list = run(["memory", "list", "--status", "trashed", "--json", "--repo-key", "test-trash"]);
		const items = JSON.parse(list.stdout);
		expect(items.some((i: { id: string }) => i.id === id)).toBe(true);
	});
	...
	it("memory untrash restores trashed memory to active", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Untrash test", "--body-file", bodyFile, "--repo-key", "test-untrash"]);
		const id = rec.stdout.trim();

		run(["memory", "trash", id, "--reason", "temp", "--repo-key", "test-untrash"]);
		const ut = run(["memory", "untrash", id, "--repo-key", "test-untrash"]);
		expect(ut.status).toBe(0);

		const list = run(["memory", "list", "--status", "active", "--json", "--repo-key", "test-untrash"]);
		const items = JSON.parse(list.stdout);
		expect(items.some((i: { id: string }) => i.id === id)).toBe(true);
	});
	...
	it("memory purge without --yes exits with code 1", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Purge test", "--body-file", bodyFile, "--repo-key", "test-purge"]);
		const id = rec.stdout.trim();
		run(["memory", "trash", id, "--reason", "prep", "--repo-key", "test-purge"]);

		const pr = run(["memory", "purge", id, "--reason", "test", "--repo-key", "test-purge"]);
	...
	it("memory purge with --yes succeeds", () => {
		const bodyFile = path.join(cacheHome, "body.md");
		fs.writeFileSync(bodyFile, "purge body");
		const rec = run(["memory", "record", "--type", "decision", "--title", "Purge yes test", "--body-file", bodyFile, "--repo-key", "test-purge-yes"]);
		const id = rec.stdout.trim();
		run(["memory", "trash", id, "--reason", "prep", "--repo-key", "test-purge-yes"]);

		const pr = run(["memory", "purge", id, "--reason", "test purge", "--yes", "--repo-key", "test-purge-yes"]);
	...
	it("memory link creates a link between two memories", () => {
		...
		const src = run(["memory", "record", "--type", "decision", "--title", "Link src", "--body-file", b1, "--repo-key", "test-link"]).stdout.trim();
		const dst = run(["memory", "record", "--type", "decision", "--title", "Link dst", "--body-file", b2, "--repo-key", "test-link"]).stdout.trim();

		const lk = run(["memory", "link", src, dst, "--type", "supports", "--repo-key", "test-link"]);
	...
	it("memory link without --type exits with code 1", () => {
		...
		const src = run(["memory", "record", "--type", "decision", "--title", "No type src", "--body-file", b1, "--repo-key", "test-link-notype"]).stdout.trim();
		const dst = run(["memory", "record", "--type", "decision", "--title", "No type dst", "--body-file", b2, "--repo-key", "test-link-notype"]).stdout.trim();

		const lk = run(["memory", "link", src, dst, "--repo-key", "test-link-notype"]);
	...
	it("memory unlink removes link between memories", () => {
		...
		const src = run(["memory", "record", "--type", "decision", "--title", "Unlink src", "--body-file", b1, "--repo-key", "test-unlink"]).stdout.trim();
		const dst = run(["memory", "record", "--type", "decision", "--title", "Unlink dst", "--body-file", b2, "--repo-key", "test-unlink"]).stdout.trim();

		run(["memory", "link", src, dst, "--type", "supports", "--repo-key", "test-unlink"]);
		const ul = run(["memory", "unlink", src, dst, "--type", "supports", "--repo-key", "test-unlink"]);
	...
	it("memory pin succeeds", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Pin test", "--body-file", bodyFile, "--repo-key", "test-pin"]);
		const id = rec.stdout.trim();
	...
	it("memory pin then unpin succeeds", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Unpin test", "--body-file", bodyFile, "--repo-key", "test-unpin"]);
		const id = rec.stdout.trim();
	...
	it("memory confirm promotes candidate to active", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Confirm test", "--body-file", bodyFile, "--source", "extracted", "--repo-key", "test-confirm"]);
		const id = rec.stdout.trim();
	...
	it("memory audit --json returns rows with created changeType", () => {
		...
		const rec = run(["memory", "record", "--type", "decision", "--title", "Audit test", "--body-file", bodyFile, "--repo-key", "test-audit"]);
		const id = rec.stdout.trim();

		const au = run(["memory", "audit", id, "--json", "--repo-key", "test-audit"]);
		expect(au.status).toBe(0);
		const rows = JSON.parse(au.stdout);
		expect(Array.isArray(rows)).toBe(true);
		expect(rows.length).toBeGreaterThanOrEqual(1);
		expect(rows.some((r: { changeType: string }) => r.changeType === "create")).toBe(true);
	});
	...
	it("memory reconcile --report outputs JSON with expected keys", () => {
		const out = run(["memory", "reconcile", "--report", "--repo-key", "test-reconcile-report"]);
		expect(out.status).toBe(0);
		const parsed = JSON.parse(out.stdout);
